...
int usb_dwc3_export_state(dwc3_dev_t *dev, struct usb_dwc3_export_state *state)
{
    ...
    if (!dev || !state || !dev->pipe[CDC_ACM_PIPE_1].ready)
        return -1;

    tx_endpoint = dev->pipe[CDC_ACM_PIPE_1].ep_in;
    rx_endpoint = dev->pipe[CDC_ACM_PIPE_1].ep_out;
    device2host = dev->pipe[CDC_ACM_PIPE_1].device2host;

    usb_dwc3_handle_events(dev);
    usb_dwc3_cdc_start_bulk_in_xfer(dev, tx_endpoint);
    if (ringbuffer_get_used(device2host))
        return -1;
    usb_dwc3_cdc_start_bulk_out_xfer(dev, rx_endpoint);

    memset(state, 0, sizeof(*state));
    state->regs_phys = dev->regs;
    state->event_buffer_phys = (u64)dev->evtbuffer;
    state->event_buffer_size = DWC3_EVENT_BUFFERS_SIZE;
    state->event_buffer_offset = dev->evt_buffer_offset;
    state->scratchpad_phys = (u64)dev->scratchpad;
    state->scratchpad_size = max(DWC3_SCRATCHPAD_SIZE, SZ_16K);
    state->xfer_buffer_phys = (u64)dev->xferbuffer;
    state->xfer_buffer_size = XFER_BUFFER_SIZE;
    state->trb_buffer_phys = (u64)dev->trbs;
    state->trb_buffer_size = TRB_BUFFER_SIZE;
    state->tx_buffer_phys = (u64)dev->endpoints[tx_endpoint].xfer_buffer;
    state->tx_buffer_iova = dev->endpoints[tx_endpoint].xfer_buffer_iova;
    state->tx_trb_phys = (u64)dev->endpoints[tx_endpoint].trb;
    state->tx_trb_iova = dev->endpoints[tx_endpoint].trb_iova;
    state->tx_endpoint = tx_endpoint;
    state->tx_max_packet = XFER_SIZE;
    state->tx_busy = dev->endpoints[tx_endpoint].xfer_in_progress;
    state->rx_buffer_phys = (u64)dev->endpoints[rx_endpoint].xfer_buffer;
    state->rx_buffer_iova = dev->endpoints[rx_endpoint].xfer_buffer_iova;
    state->rx_trb_phys = (u64)dev->endpoints[rx_endpoint].trb;
    state->rx_trb_iova = dev->endpoints[rx_endpoint].trb_iova;
    state->rx_endpoint = rx_endpoint;
    state->rx_busy = dev->endpoints[rx_endpoint].xfer_in_progress;

    page_table_count = dart_collect_page_tables(dev->dart, state->page_tables,
                                                ARRAY_SIZE(state->page_tables));
    if (!page_table_count || page_table_count > ARRAY_SIZE(state->page_tables))
        return -1;
    state->page_table_count = page_table_count;
